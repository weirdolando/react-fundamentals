...
In this exercise, we use unpkg so we can use React. Notice in the unpkg link
there's a UMD. UMD stands for (Universal Module Definition), it allows us to
create global variables in the browser, thus we can use React and ReactDOM
without installing it. UMD is just one of many modules in JavaScript such as
CommonJS, ES Modules, etc. More
[here](https://jameshfisher.com/2020/10/04/what-are-umd-modules/).

Note: React separates the `React` and `ReactDOM` packages because there are
other frameworks in React for different environment which are React Native
(mobile) and React VR. They separate it so we can still use the core `React`
package in different environment. `ReactDOM` is only for website.
