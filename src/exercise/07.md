...
Each time React renders your components, it's calling your functions to retrieve
the new React elements that it uses to update the DOM. If you return the same
element types, it **keeps** those components/DOM nodes around, **even if all the
props changed**.

The exception to this is the `key` prop. This allows you to return the exact
same element type, but **force** React to unmount the previous instance, and
mount a new one if the `key` is changed. This means that all state that had
existed in the component at the time is completely removed and the component is
"reinitialized" for all intents and purposes. For components, this means that
React will run cleanup on effects (or `componentWillUnmount`), then it will run
state initializers (or the `constructor`) and effect callbacks (or
`componentDidMount`).
...
function App(props) {
  ...
  return (
    <>
      ...
      {/* Greet won't be rerendered every time state changes */}
      {props.greet}
    </>
  )
}

ReactDOM.createRoot(root).render(<App greet={<Greet children="Morning" />} />)
```

Here, _Greet_ won't be rerendered because we pass it as a props to the _App_
component. Thus, when the _App_ component rerenders, _Greet_ props object is
still the same object (since we're not creating a new Greet element). While
_Message_ component props object is created on every renders. More
[here](https://kentcdodds.com/blog/optimize-react-re-renders)
