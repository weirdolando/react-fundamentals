...
If you have an event handler, the `event` object in React is actually not the
native DOM `event`, but it's a React `SyntheticEvent`. In general, it has all
the exact same API as the DOM Event. React use this synthetic event for
performance issues. However, you can also access the native DOM event by saying
`event.nativeEvent`.

### `event.preventDefault` and Form

By default, if you have a form and you don't run this function when the form is
submitted, the browser will send a GET method to the current URL and use the
form controls values as query parameters.

### `console.dir`

We can use `console.dir` to print a javascript object in a directory structure.
Let's say you select a `form` element from the DOM. When you `console.log` it,
it will show you the DOM element and its children, but when you use
`console.dir`, it will show the the `form` object.
...
Here is a controlled input. The value of the username input is stored in the
state, and we change the input value by changing the state value. If you remove
the `onChange` handler from the input, the input value won't change even the
user type something in the input (not mutable). React will also give you a
warning that the input will be rendered as a read-only field. If this is the
behavior that you're looking for, and you want to remove the warning, just add
`readOnly` property to the input.
...
specify a `defaultValue` attribute instead of value. Changing the value of
defaultValue attribute after a component has mounted will not cause any update
of the value in the DOM.
