...
### Diference between HTML and JSX

There are differences between HTML and JSX. `for` becomes `htmlFor`, `class`
becomes `className`, `innerHTML` becomes `dangerouslySetInnerHTML`, etc. More
[here](https://reactjs.org/docs/dom-elements.html#differences-in-attributes).

### Babel

When we use Babel, and we put our JSX code inside the
`<script type="text/babel"></script>` tag, the browser won't actually run that
`<script>`. The one who runs that `<script>` is Babel. It will create a new
`<script>` inside the `<head>` tag with our JSX code that has already compiled
to JavaScript. Note: don't use this for production.
...
Here, when doing spread operator on the `element`'s attribute, Babel will do
`Object.assign` to both of these objects which are `{id: "foo"}` and `props`.
